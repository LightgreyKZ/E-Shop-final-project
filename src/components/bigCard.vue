<template>
  ...
  <div class="card__container">
    ...
    <div class="card__container_info">
      <div class="card__container_info-title">
        {{ localGoodsArray.description }}
      </div>
      <div class="card__container_info-stock" :class="SetStockColor">
        {{ inStockOrder }}
      </div>
      <div class="colors__container">
        <!-- Иконки выбора цвета. Меняют границу у изображения товара. Определяется какой цвет выбран  -->
        <button
          class="colors__container_colors"
          :class="{ blackborder: SelectedColor == 'beige' }"
          @click="SetItemColor('beige')"
        >
          <div class="colors__container_colors-inside color1"></div>
        </button>
        <button
          class="colors__container_colors"
          :class="{ blackborder: SelectedColor == 'blue' }"
          @click="SetItemColor('blue')"
        >
          <div class="colors__container_colors-inside color2"></div>
        </button>
        <button
          class="colors__container_colors"
          :class="{ blackborder: SelectedColor == 'purple' }"
          @click="SetItemColor('purple')"
        >
          <div class="colors__container_colors-inside color3"></div>
        </button>
        <button
          class="colors__container_colors"
          :class="{ blackborder: SelectedColor == 'grey' }"
          @click="SetItemColor('grey')"
        >
          <div class="colors__container_colors-inside color4"></div>
        </button>
        <span>Выбран: {{ SelectedColor }}</span>
      </div>
      <div class="card__container_info-oldprice" v-if="percentDiscount">
        <s>{{ localGoodsArray.price }}&#36;</s>
      </div>
      <div
        class="card__container_info-price"
        :class="{ redPrice: percentDiscount }"
      >
        {{
          localGoodsArray.price -
          ((localGoodsArray.price * percentDiscount) / 100).toFixed(2)
        }}&#36;
      </div>
      <button class="card__container_info-buttoncart" @click="add_To_Cart()">
        Добавить в корзину
      </button>
    </div>
  </div>
