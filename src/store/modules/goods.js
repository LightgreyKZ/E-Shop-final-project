  ...
  getters: {
    GET_DISCOUNT_BY_ID(state) {
      return function (payload) {
        ...
        const indexItem = state.gotDiscounts.findIndex(
    ...
    cartTotalDiscount(state, getters, rootState, rootGetters) {
      const totalDiscount = rootState.cartArray.reduce((sum, item) => {
        return (
          ...
          item.price *
            item.quantity *
            (getters.GET_DISCOUNT_BY_ID(item.id) / 100)
        );
      }, 0);
    ...
    GET_NAME_BY_ID(state) {
      return function (payload) {
        const indexItem = state.gotGoods.findIndex(
          (item) => item.id == payload
        );
        if (indexItem !== -1) {
  ...
  actions: {
    ...
    async getDiscounts({ commit }) {
      try {
        ...
        alert(
          "Ошибка при получении списка скидок! Нужно запустить json-server: `npm install -g json-server`, затем json-server src/data/dis.json"
        );
      }
    },
